...
void on_trackbar(int, void*){
  ...
  gammaL = (double)gammaL_slider/10.0; // gammaL é um real e varia de 0 a 1
  c = (double)c_slider; // c é um real e varia de 0 a 60
  D0 = (double)D0_slider;
  if(D0<1.0){
    D0=1.0;
  }
  ...
  filtrada = filtra();
  imshow("filtrada", filtrada); // a imagem precisa estar no escopo global
  normalize(filtrada, filtrada, 0, 255, CV_MINMAX); // normaliza a parte real para exibicao
...
int main(int , char**){
  ...
  vector<String> end;
  glob("/home/victor/Área de Trabalho/victorers1.github.io/img/sandalia.jpg", end, false);
  image = imread(end[0], CV_LOAD_IMAGE_GRAYSCALE);
  resize(image, image, cv::Size(516, 688));  // Redimensionamos a imagem por ser muito grande
